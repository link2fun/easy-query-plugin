 ...
 * @author xuejiaming
 */
public class APTVersion2_2 {
    private static final Map<String, PropertyColumn> TYPE_COLUMN_MAPPING = new HashMap<>();
    private static final Logger log = Logger.getInstance(APTVersion2_2.class);
...
//                                    aptFileCompiler.addImports("com.easy.query.core.proxy.columns.SQLQueryable");
                        ...
                        ApplicationManager.getApplication().invokeLater(() -> {
                            try {
                                PsiFile psiProxyFile = VelocityUtils.render(project, context, Template.getTemplateContent("AptTemplate2_2" + suffix), proxyEntityName + suffix);
                                ...
                                log.error("生成代理文件时发生错误", e);
                            }
                        });

                    } catch (Exception e) {
                        log.error("PSI操作时发生错误", e);
                    }
                });
            });
        });
...
//        if(propertyColumn.getPropertyType().equals("com.easy.query.test.entity.school.MySchoolClass1")){
        ...
        // 这个方法已经在ReadAction中被调用，所以不需要额外包装
